...
Both citation shapes SYSTEM_INSTRUCTION (generation/answer.py) enforces
...
def _build_citation_pattern(law_names):
    """
...
def parse_citations(answer_text, results):
    if not answer_text or not results:
        return []

    law_names = {r["law_name"] for r in results}
    pattern = _build_citation_pattern(law_names)
    if pattern is None:
